...
function fixVideo(video: Video) {
  return {
    ...
    mirrors: (video.mirrors || []).map((mirror) => {
      ...
      return {
        ...mirror,
        url: mirror.url.startsWith('http')
          ? mirror.url
          : `https://archive.speedrun.club/cdn/${video.id}.m3u8`,
      }
    }),
  }
}

const AfterSchema = z.tuple([VideoSchema.shape.id, z.string()])
...
export async function searchVideos(
...
): Promise<PaginatedResponse<VideoOutput>> {
  ...
  if (isNumeric) {
    ...
    const videoIdMatch = params.query.match(
      /twitch\.tv\/(?:videos\/|\w+\/[bcv]\/|.+video=v?)(\d+)/i,
    )
  ...
  const _results = VideoQuerySchema.parse(await query<VideoQuery>(fullQuery, values))
  ...
  const channelResponses = await Promise.all(
    chunk(resultChannelIds.map(String), 100).map((ids) => getUsers({ id: ids })),
  )

  const channelMap = new Map(
    channelResponses.flatMap((response) => response.data.map((user) => [parseInt(user.id), user])),
  )
...
export async function getVideoById(id: number): Promise<VideoOutput | undefined> {
  ...
  const {
    data: [channel],
  } = await getUsers({ id: [String(result.channel_id)] })
...
export async function addVideoFromMetadata(
  ...
  const type = v.broadcastType.includes('PREMIERE')
    ? 'upload'
    : (v.broadcastType.toLowerCase() as VideoType)
...
export async function replaceMirror(
  ...
  await query(`UPDATE mirrors SET url = $3 WHERE id = $1 AND source = $2`, [videoId, source, url])
}
...
export async function findMirror(videoId: number): Promise<string | undefined> {
  ...
  if (!video) {
    ...
    const videoMetadataUrl = applyIaPrefix(
      `https://gql.twitch.tv/gql?operationName=VideoMetadata&variables_channelLogin=${username}&variables_videoID=${videoId}`,
      iaPrefix,
    )
  ...
  */
  ...
  const findFromJson = async () => {
    const usherUrlRepsonse = await fetchJson(
      `https://archive.org/wayback/available?url=https://usher.ttvnw.net/vod/${videoId}.m3u8*`,
      undefined,
      100,
    )
  ...
  const findFromCdx = async () => {
    const cdxResponse = await fetchText(
      `https://web.archive.org/cdx/search/cdx?url=usher.ttvnw.net/vod/${videoId}.m3u8*&limit=-1&fastLatest=true`,
      undefined,
      100,
    )
